...
port_no = 6;	// This is the port no where the Zigbee is conneted ( COM 9 ) . You may have to change the value according to the COM port you are using.

inputImageBuffer = imread('C:\Users\ERTS Lab\Desktop\mano\CS684Team1(10305903,10305024,10305022)\Source Code\Scilab Code\test22.bmp')  // This is the location of the Input image.

[rows,cols] = size(inputImageBuffer);
color1=zeros(1,rows*cols);
count=zeros(1,rows*cols);
inputImageBuffer2=zeros(rows,cols,3);
inputImageBuffer3=zeros(rows,cols,3);
x=5;		// Color value
y=-30;		// row position
i=0;
r=1;
...
for td = 1:3
  for r=1:rows
  	for c=1:cols
  		if inputImageBuffer(r,c,td)==255 | inputImageBuffer(r,c,td) == 254 | inputImageBuffer(r,c,td) == 253 then,
  			inputImageBuffer2(r,c,td)=1;
//  		else
//  			inputImageBuffer(r,c,td)=0;
  		end
  	end,
  end,
end,
  ...
  for r=1:rows
  	for c=1:cols
  		if modulo(r,2) == 1 then,
  			inputImageBuffer2(r,c,td)=inputImageBuffer(r,c,td);
  		else
  			inputImageBuffer2(r,c,td)=inputImageBuffer(r,cols-c+1,td);
  		end
  	end,
  end,
end,
...
for r=1:rows
	for c=1:cols
		if (inputImageBuffer(r,c,td)==x & y==r & z_f == 'F') | (inputImageBuffer(r,c,1) == x & inputImageBuffer(r,c,2) == x & inputImageBuffer(r,c,3) == x & y == r & z_f == 'T') then,
			count(i)=count(i)+1;
		else
		  if inputImageBuffer(r,c,1) == 1 then,
		    color1(j) = 1;
		    td = 1;
		    z_f = 'F';
		  else
		   if inputImageBuffer(r,c,2) == 1 then,
		    color1(j) = 2;
		    td = 2;
		    z_f = 'F';
		   else
		    if inputImageBuffer(r,c,3) == 1 then,
		      color1(j) = 3;
		      td = 3;
		      z_f = 'F';
		    else
